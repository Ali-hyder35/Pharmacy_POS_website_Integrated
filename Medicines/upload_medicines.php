...
if (isset($_POST['upload'])) {
    ...
    while (($row = fgetcsv($handle, 1000, ",")) !== false) {
        ...
        // Map columns safely
        $medicine_name     = isset($row[1]) ? $row[1] : '';
        $generic           = isset($row[2]) ? $row[2] : '';
        $dosage_form       = isset($row[3]) ? $row[3] : '';
        ...
        $sql = "CALL PRC_MEDICINE_ADD(
            '".addslashes($medicine_name)."',
            '".addslashes($generic)."',
            '".addslashes($dosage_form)."',
            '".addslashes($category)."',
            '".addslashes($rack)."',
            '".addslashes($price)."',
            '".addslashes($stock)."',
