...
if (isset($_GET['delete'])) {
    ...
    $stmt = $conn->prepare("CALL PRC_PRODUCTS_DEL(?, @PCODE, @PDESC, @PMSG)");
    ...
    header("Location: SEARCH_COSMETICS.php?page=$page&search=" . urlencode($search));
    exit;
}
...
if ($name !== '' || $category !== '' || $batch !== '') 
    {
    $stmt = $conn->prepare("
        CALL PRC_SEARCH_PRODUCTS(
            NULL, ?, ?, ?, 
            @PCODE, @PDESC, @PMSG
        )
    ");
        
    $stmt->bind_param("sss", $name,  $category, $batch);
    ...
    {
        $stmt = $conn->prepare("CALL PRC_SEARCH_ALL_PRODCUTS(@PCODE, @PDESC, @PMSG)");
    }
...
$total_records = count($cosmetics);
$total_pages   = ceil($total_records / $limit);
$cosmetics     = array_slice($cosmetics, $offset, $limit);

/* ================= LOW STOCK ================= */

$lowStockItems = array_filter($cosmetics, fn($m) => $m['remaining_stock'] <= 10);
...
<head>
<meta charset="UTF-8">
<title>All cosmetics | Pharmacy POS</title>
...
<style>
 ...
 .col-md-3 .filter-box, .col-md-9 .records-box 
 {
     height: 100%; 
    }
 .modal-header 
...
<div class="topbar bg-white p-3 d-flex justify-content-between shadow-sm">
    <strong>SEARCH cosmetics</strong>
...
<tr><td colspan="12" class="text-center text-muted">No cosmetics found</td></tr>
<?php else: foreach($cosmetics as $m): ?>
<tr>
    ...
    <td><?= htmlspecialchars($m['updated']) ?></td>
          ...
          <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header bg-primary text-white">
                <h5 class="modal-title">Edit Cosmetics: <?= htmlspecialchars($m['name']) ?></h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6"><label class="form-label">Company Name</label>
                            <input type="text" name="Company_Name" class="form-control" value="<?= htmlspecialchars($m['Company_Name']) ?>"></div>
                        ...
                        <div class="col-md-6"><label class="form-label">Updated</label>
                            <input type="text" name="updated" class="form-control" value="<?= htmlspecialchars($m['updated']) ?>"></div>


                                                        
                        <div class="col-md-6"><label class="form-label">Updated_by</label>
                            <input type="text" name="updated_by" class="form-control" value="<?= htmlspecialchars($m['updated_by']) ?>"></div>
